...
证据核查日期：**2026-09-07**。本文为 MedSegAgent 的工程选型记录；只检索官方论文、发行包、源码、模型卡、许可证与公开数据元信息。本文形成时尚未安装或推理任何补充权重，不代表 Mac、pjoffice 或公网已支持这些能力。后续真实验收记录优先于本研究快照。

## 建议先选两个 CT 病灶任务

优先对 **TotalSegmentator `lung_nodules` 与 `liver_lesions`** 做独立 MPS/CUDA canary，再决定是否开放。二者补足基础器官分割缺少的肺结节与肝病灶能力，复用已经存在的本地推理依赖、隔离目录、任务队列和结果契约。暂不为了数量增加第三个模型。

这里“推荐”表示值得进入实测门槛，**不表示两个平台已经通过**。用户要求的“两平台实际安装和运行”只能由后续真实任务满足，不能用 PyPI 可下载、`device='mps'` 参数存在或论文成绩代替。
