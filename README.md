...
Segment CT and MR images with natural-language requests, powered by
[TotalSegmentator](https://github.com/wasserth/TotalSegmentator). Explore the results
in an interactive 3D workspace and download the masks you need.

[Live demo](https://medseg.huangziyan97.com) · [Deployment](docs/deployment.md) ·
...
- **Flexible workflows** — combine models, inspect region volumes and create
...
- **One core, four interfaces** — Web, CLI, MCP and A2A, with local model inference.
...
```bash
git clone --branch totalseg-agent https://github.com/uni-medical/MedSegAgent.git
cd MedSegAgent
uv sync --frozen
cp .env.example .env
```
...
This branch contains TotalSeg Agent. The MedSegAgent paper and original experiments
